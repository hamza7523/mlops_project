...
def train_cv_model():
    ...
    _train_transforms = Compose([
        RandomResizedCrop(size), RandomHorizontalFlip(), RandomRotation(15),
        ColorJitter(brightness=0.1, contrast=0.1), ToTensor(), normalize,
    ])
    _val_transforms = Compose([Resize(size), CenterCrop(size), ToTensor(), normalize])

    def transform_train(example):
        example["pixel_values"] = [_train_transforms(Image.open(x).convert("RGB")) for x in example["image"]]
        example["label"] = [label2id[y] for y in example["label"]]
        return example

    def transform_val(example):
        example["pixel_values"] = [_val_transforms(Image.open(x).convert("RGB")) for x in example["image"]]
        example["label"] = [label2id[y] for y in example["label"]]
        return example
    ...
    model = AutoModelForImageClassification.from_pretrained(
        CV_MODEL_CHECKPOINT, num_labels=len(classes), id2label=id2label, label2id=label2id, ignore_mismatched_sizes=True
    )
...
def build_rag_db():
    ...
    with mlflow.start_run(run_name="RAG_Ingestion_Setup"):
        ...
        if os.path.exists("flora_rag_db"): shutil.rmtree("flora_rag_db")
        vectordb = Chroma.from_documents(docs, embedding_model, persist_directory="flora_rag_db")
        vectordb.persist()
